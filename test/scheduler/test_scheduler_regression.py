...
@pytest.fixture
def scheduler_env(db_url, setup_test_db, tmp_path, monkeypatch):
...
@pytest.fixture
def seed_team_and_category(db_session):
...
def test_execute_job_auto_pauses_expired_job_and_removes_schedule(
    scheduler_env, seed_team_and_category, db_session, setup_test_db
):
    ...
    with get_db_session() as session:
        persisted = session.get(Job, expired_job.id)
